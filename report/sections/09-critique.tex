...
\paragraph{What saturates is a population, not a person.} The unit of this work
is one day per user, so what the model learns is how a \emph{population} moves.
The flatness of the scaling curve should be read in that light: a population
prior is a much smaller object than a personal habit, and it is unsurprising
that it is bought early.

\paragraph{Corpus-to-corpus comparisons are deltas only.} The test population
changes whenever the corpus does, and with it the baseline, so two runs on two
corpora cannot be compared on absolute accuracy at all. This is why every figure in this report is a gap, and
