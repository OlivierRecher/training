...
Three encoding choices are implemented in \textbf{\panelref{fig:training-example}{b}}:
...
\textbf{Figure~\ref{fig:protocol}} illustrates this evaluation across a 24-hour subscriber trajectory: the prefix and suffix split in \textbf{\panelref{fig:protocol}{a}}, and the step-by-step scoring of the suffix in \textbf{\panelref{fig:protocol}{b}}.

\input{figures/fig-topk}

\textbf{Figure~\ref{fig:topk}} demonstrates the multi-depth evaluation procedure. For each prediction step in \textbf{\panelref{fig:topk}{a}}, candidate cell tokens are ranked by output probability, recording top-1, top-3, and top-5 accuracy. Top-1 represents strict exact-match prediction, while top-3 and top-5 evaluate candidate list coverage in the same \textbf{\panelref{fig:topk}{a}}. \textbf{\panelref{fig:topk}{b}} shows how these right-or-wrong answers are counted into a score.
...
This principle is the one drawn in \textbf{\panelref{fig:three-rules}{a}}. Fine-tuning small datasets leads to overfitting when trained for excessive epochs. As shown in \textbf{Figure~\ref{fig:validation-curves}}, validation accuracy on unseen held-out subscribers reaches a peak before declining.

In an extended 49-epoch test run shown in \textbf{\panelref{fig:validation-curves}{a}}, training loss decreased continuously, whereas held-out validation accuracy peaked at epoch 9 (64.2\%) before declining to 48.7\% at epoch 49. Evaluating at final epoch completion would incur a \textbf{15.5 percentage point loss} in accuracy. Implementing early stopping at peak validation accuracy alongside regularization (dropout 0.1, weight decay, learning rate scheduling) increased held-out accuracy from 48.7\% to \textbf{64.4\%} while reducing training time from 72 to 29 minutes. \textbf{Figures~\panelnum{fig:validation-curves}{b}}, \textbf{\panelnum{fig:validation-curves}{c}}, and \textbf{\panelnum{fig:validation-curves}{d}} confirm that this optimization dynamic holds consistently across dataset splits. All subsequent reported model metrics evaluate the optimal checkpoint saved at peak validation accuracy.
...
This principle is the one drawn in \textbf{\panelref{fig:three-rules}{b}}. The prefix context fraction (share of trajectory provided to the model prior to prediction) directly affects task difficulty and baseline performance. \textbf{Table~\ref{tab:context-sweep}} presents model performance across varying context fractions (30\% to 90\%) on \Dfilt{}. While the model achieves 80.1\% accuracy at an 80\% context fraction against a 79.0\% persistence baseline (+1.1\%) in \textbf{Table~\ref{tab:context-sweep}}, evaluating across context fractions reveals that at a 70\% context fraction the model trails the baseline by 0.3\%. Consequently, context fraction choices must be fixed across comparative evaluations.
