...
Data-preparation pipeline           & data preparation   & the train/test JSONL files, timestamps kept as hours; \seleven{} as one merged training file with nested test sets \\
Behavioural-group analysis          & analysis           & the four profiles and the derived windows of Chapter~\ref{ch:groups} \\
\bottomrule
\end{tabular}

\figtext{%
The artefacts, and what each one produces.}
\end{table}
...
\begin{table}[t]
\centering\footnotesize
\caption{Configuration keys carrying a decision}
...
\figtext{%
Configuration values that encode a methodological choice rather than a tuning
preference.}
\end{table}
