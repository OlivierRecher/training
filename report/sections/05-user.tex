...
\textbf{Figure~\ref{fig:concentration}} ranks candidate subscriber-level metrics by Pearson correlation coefficient $r$ evaluated across 1,173 subscribers on \Draw{} (dark blue) and 100 held-out subscribers on \Dmob{} (teal). The primary determinant of predictability is cell transition frequency, formalized as the \textbf{cell-switch rate}. For a given subscriber trajectory, the cell-switch rate measures the proportion of consecutive event records registering a change in cell identifier. Stationary subscribers exhibit cell-switch rates near 0\%, whereas highly mobile subscribers approach 100\%. Cell-switch rate demonstrates a strong inverse correlation with prediction accuracy ($r = -0.971$ across raw log subscribers and $r = -0.773$ across held-out mobile subscribers): \emph{higher cell transition frequency strongly correlates with lower prediction accuracy}. Complementary measures of trajectory repetitiveness---such as primary cell concentration concentration ($r = +0.696$) and transition temporal entropy ($r = -0.811$)---reflect the same underlying property from alternative perspectives.

\textbf{Figure~\ref{fig:concentration}} also demonstrates that alternative candidate features exhibit minimal correlation with model accuracy ($|r| < 0.24$): unique cell count ($r = -0.232$), prediction instance count ($r = -0.101$), and total trajectory length ($r = +0.054$) exert negligible influence on individual predictability. Because a subscriber's cell-switch rate is the arithmetic mirror image of persistence baseline accuracy, high baseline repetitiveness inherently yields a strong negative correlation coefficient.

\subsection{Quintile Stratification Analysis}

\input{figures/fig-switch-rate}

\textbf{Figure~\ref{fig:switch-rate}} validates this dependency without relying exclusively on scalar correlation coefficients by stratifying subscribers into equal mobility cohorts based on cell-switch rate. \textbf{\panelref{fig:switch-rate}{a}} evaluates 500 subscribers on \Draw{} partitioned into quintiles, while \textbf{\panelref{fig:switch-rate}{b}} evaluates 100 held-out subscribers on \Dmob{} partitioned into quartiles.
...
\textbf{\panelref{fig:switch-rate}{a}} highlights model performance across cell-switch quintiles on \Draw{}. For the lowest cell-switch quintile (subscribers who never change cell ID), both the fine-tuned model and the persistence baseline achieve 100\% prediction accuracy. Across the three lowest cell-switch quintiles (low mobility), fine-tuned model accuracy and persistence baseline performance remain virtually identical. Model accuracy gains over the baseline manifest exclusively within the two highest cell-switch quintiles (high mobility), exceeding baseline performance by +3.7 percentage points (56.5\% vs. 52.8\%) and +7.8 percentage points (34.4\% vs. 26.6\%), respectively.

However, high cell-switch subscribers account for only 18.5\% of total evaluated prediction instances in \textbf{\panelref{fig:switch-rate}{a}}, with the highest cell-switch quintile representing 5.6\% of total volume. Achieving a +7.8 percentage point improvement on 5.6\% of total evaluation volume yields a negligible shift in aggregate population accuracy. On the unconditioned mobile cohort in \textbf{\panelref{fig:switch-rate}{b}}, the model trails the baseline across the three lower mobility quartiles (-7.1\%, -5.3\%, and -3.4\%) and draws level (+0.6\%) only in the highest mobility quartile. This establishes a key reporting convention: \emph{aggregate population accuracy averages distinct mobility cohorts; reporting segment-specific accuracy relative to baseline benchmarks is required to capture true model contributions.}
...
\textbf{Table~\ref{tab:worst-users}} details the seven lowest-scoring subscriber trajectories ($\le 12.5\%$ accuracy) on \Dfilt{}. Analysis partitions these worst-case trajectories into three distinct failure modes:
1. \textbf{Low Record Volume} (Rows 1--4): subscribers contributing between 2 and 12 evaluation moments, where small sample sizes introduce high variance (e.g. 3 moments yield discrete scores of 0\%, 33\%, 67\%, or 100\%).
2. \textbf{Genuine Trajectory Change} (Row 5): a true subscriber behavioral shift where baseline and model both fail (ceiling 89\%, baseline 67\%, model 11.1\%).
3. \textbf{Intra-Site Sector Hand-offs} (Rows 6--7): subscribers oscillating between adjacent directional sector antennas mounted on the same physical base station mast (e.g. \cid{UKVCHE102} and \cid{UKVCHE104}). For these trajectories, the theoretical ceiling is 100\% and the persistence baseline achieves 50\%--67\% accuracy by repeating sector IDs; the model attempts to forecast specific sector switching, scoring 0\% despite zero physical spatial displacement.
