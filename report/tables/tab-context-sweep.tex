...
\figtext{%
\figpara{What was varied, and what the columns are.}{Only where each person's day
is cut into the part the model reads and the part it must produce: ``70 / 30''
means the model sees the first 70\,\% and is graded on the last 30\,\%.
...
antenna'' scores on those very same moments, and \textbf{Distance} the model
minus the rule, which is the only column that means anything.}

\figpara{The point of the table is the \textbf{Rule} column.}{It changes from row
to row. Moving the cut changes which antenna counts as ``the last one seen'', so
it moves the competitor too, and each row must be read against the rule on its
own line. On \sfilt{} exactly one cut out of three clears the rule, by 1.1, while
...
fast. Each \sfilt{} row is a separate training run; the \smob{} block is one
model read seven times, and the giveaway is its identical \textbf{Valid.}
column.}}
