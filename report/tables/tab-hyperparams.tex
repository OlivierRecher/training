...
\figtext{%
\figpara{Hyperparameter configurations across experimental iterations in Table~\ref{tab:hyperparams}.}{One column per family of runs in Table~\ref{tab:hyperparams}, in chronological order.
...
\figpara{Key training hyperparameter settings in Table~\ref{tab:hyperparams}.}{\textbf{Max passes} with \textbf{Early
stopping} in Table~\ref{tab:hyperparams} is the rule of \textbf{Chapter~\ref{ch:tools}}: twenty passes are allowed, the run
stops as soon as its score on unseen people stops improving, and the best copy is
kept. \textbf{Context fraction} at 0.8 is where each day is cut, and belongs to the
measurement rather than here. \textbf{Emphasised hours} is where
\textbf{Chapters~\ref{ch:time}} and~\textbf{\ref{ch:groups}} differ, and it is the only substantive
change in the last two columns.}}
