% T12 -- Hyperparameters, per run family.
\begin{minipage}{\textwidth}
\centering\footnotesize
\captionof{table}{Hyperparameters per run family}
\label{tab:hyperparams}
\begin{tabular}{@{}l@{\hspace{6pt}}>{\raggedright\arraybackslash}p{2.6cm}@{\hspace{6pt}}>{\raggedright\arraybackslash}p{2.9cm}@{\hspace{6pt}}>{\raggedright\arraybackslash}p{2.9cm}@{\hspace{6pt}}>{\raggedright\arraybackslash}p{3.1cm}@{}}
\toprule
& \textbf{Iter.\ 1} & \textbf{Iter.\ 2--6} & \textbf{Iter.\ 7--9} & \textbf{Iter.\ 10--11} \\
\midrule
Base model        & Mistral-7B-v0.1 & Qwen2.5-0.5B-Instruct & Qwen2.5-0.5B-Instruct & Qwen2.5-0.5B-Instruct \\
Loading           & 4-bit (QLoRA)   & bf16 / fp32           & bf16 / fp32           & bf16 / fp32 \\
LoRA rank $r$     & 16              & 16                    & 16                    & 16 \\
LoRA $\alpha$     & 32              & 32                    & 32                    & 32 \\
LoRA dropout      &                 & 0.1                   & 0.1                   & 0.1 \\
Learning rate     & default         & $1.5\times10^{-4}$    & $1.5\times10^{-4}$    & $1.5\times10^{-4}$ \\
Weight decay      &                 & 0.01                  & 0.01                  & 0.01 \\
Max epochs        & 100 steps       & 20                    & 20                    & 20 (cosine horizon) \\
Early stopping    &                 & patience 8, then 4    & patience 4            & 4 evaluations \\
Eval.\ sequence length &            & 512 tokens            & 512 tokens            & 512 tokens \\
Training window   &                 & 128 events, stride 64 & 128 events, stride 64 & 128 events, stride 64 \\
Context fraction  &                 & 0.8                   & 0.8                   & 0.8 \\
Added tokens      & cell ids        & 369 cells             & 369 cells + 24 hours  & 369 cells + 24 hours \\
Transition sampler &                &  $\times3$ from iter.\ 6 & $\times3$             & $\times3$ \\
Loss windows      &                 & global (4--6, 18--20) & per behavioural group & per behavioural group \\
Behavioural groups &                &                       & 4, detected from prefix & 4, refitted per tier \\
Stopping protocol & fixed steps     & epochs                & epochs                & 1{,}000 optimiser steps \\
Seed              &                 & 42                    & 42                    & 42 \\
Hardware          & Colab T4        & Colab T4              & Colab T4 / L4         & Colab GPU, 42 min/tier \\
\bottomrule
\end{tabular}

\figtext{%
Training configuration. The right-hand columns differ only where an experiment
required it; everything else was held constant on purpose, so that a
difference between two runs is a difference in the data and not in the recipe.
A blank cell means the setting did not exist in that run family: iteration~1 was
a hundred-step probe on library defaults, and the hour and group machinery only
appears from iterations~6 and~7.}
\end{minipage}
