...
\captionof{table}{Where each number quoted in the report comes from}
...
\figtext{%
\figpara{What this table is for.}{Every number quoted anywhere in the body can be
traced back to one run through this table. The \textbf{Data} column names the set
of people it was measured on, described in Table~\ref{tab:datasets}, and
\textbf{Iter.} is the numbered iteration of Appendix~\ref{app:timeline}.}

\figpara{Why the data column matters as much as the number.}{Two rows measured on
different sets of people are not comparable on their absolute scores, only on
their distances to the rule of that row's own data. Reading this table alongside
Table~\ref{tab:datasets} is what makes that check possible.}}
