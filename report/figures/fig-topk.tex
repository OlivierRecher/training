...
\node[anchor=west,font=\footnotesize\bfseries,text=clmodel] at (-0.35,3.55)
  {\panelnum{fig:topk}{a} The model never answers once. It ranks all 369 antennas.};
...
\node[anchor=west,font=\footnotesize\bfseries,text=clmodel] at (-0.35,-1.45)
  {\panelnum{fig:topk}{b} The score is then just a count, over every graded moment of every test
   person.};
...
\label{fig:topk}
\figtext{%
\figpara{\panelref{fig:topk}{a}: the model does not give an answer, it gives an order.}{Asked
...
\figpara{\panelref{fig:topk}{b}: where a percentage comes from.}{The score is a plain count.
