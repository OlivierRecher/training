...
\node[draw=clrule,fill=white,rounded corners=2pt,align=left,inner sep=6pt,
  ...
  {\textbf{What the split guarantees.} The model never sees a single test user
   before being graded, so nothing it scores is memorised. Since the unit is one
   day per user, what it learns is how a \emph{population} moves, which is the
   quantity Chapters~\ref{ch:user} and~\ref{ch:scale} measure.};
\end{tikzpicture}}
\caption{From a raw day-file to training sequences}
\label{fig:data-flow}
\figtext{%
How a raw daily export becomes training material. The 400/100 figures are
those of the mobile-user day; the other tiers of Table~\ref{tab:datasets}
