...
\node[ttl] at (0,0)
  {(c) A benchmark has to\\ leave room};
...
\end{scope}
\end{tikzpicture}}
\caption{The three rules about measuring, and what each one was worth}
\label{fig:three-rules}
\figtext{%
\figpara{Why these three come before any modelling.}{Each of the three cost me a
wrong conclusion before it was understood, and each turned out to be worth more,
in correct answers, than most of the modelling ideas that followed. None of them
is a deep idea; they are hygiene. Panel (a) is measured, on the run of
Figure~\ref{fig:validation-curves}(a). Panels (b) and (c) are drawn to show the
mechanism, and the numbers behind them are in Table~\ref{tab:context-sweep} and
Table~\ref{tab:datasets}.}

\figpara{The common shape of all three.}{In every one of them the thing that
moved was the \emph{ruler}, not the model. Reading a different pass, cutting the
day elsewhere, or scoring a different set of people all change what a given
percentage means, and none of them changes how good the predictor is. That is why
this report never quotes an accuracy without the rule that goes with it.}}
\end{figure}
