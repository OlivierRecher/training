...
\figtext{%
\figpara{Why these three come before any modelling, and what they have in
common.}{Each cost me a wrong conclusion before it was understood, and each turned
out to be worth more, in correct answers, than most of the modelling ideas that
followed. In every one of them the thing that moved was the \emph{ruler}, not the
model: reading a different pass, cutting the day elsewhere, or scoring a different
set of people all change what a given percentage means without changing how good
the predictor is. Panel (a) is measured, on the run of
Figure~\ref{fig:validation-curves}(a); panels (b) and (c) are drawn to show the
mechanism, with the numbers in Table~\ref{tab:context-sweep} and
