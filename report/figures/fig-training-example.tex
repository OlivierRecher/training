...
\figtext{%
\figpara{Panel (a) is the file on disk; panel (b) is what the model reads.}{Each
person's day is written as a record with three slots, two of them boilerplate and
the trajectory in the third, a layout chosen only for compatibility with a
library that already knows how to read it. In panel (b) the grey symbols set the
scene and are excluded from the grade; then the day arrives as pairs, an orange
symbol for the hour and a dark blue symbol for the antenna. Each antenna name is
a single symbol rather than a spelling, so the model chooses among 369 known
answers instead of spelling one letter by letter. The hour is handed over for
free and never guessed, which is legitimate here and in production alike, and it
costs one symbol rather than five: writing ``07:31:22'' would have cut the
visible history by roughly two thirds, since the model may only look at 512
symbols at a time.}}
