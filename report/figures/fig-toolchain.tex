...
\end{tikzpicture}}
\caption{The tools, and what each one does}
\label{fig:toolchain}
\figtext{%
\figpara{Read the stack from the bottom up.}{At the bottom is the machine:
a graphics card rented for free through a browser, which is the constraint
behind almost every choice in this chapter. Above it sit the model itself and
the trick that makes it fit in that card's memory. Above that, three
off-the-shelf libraries do three separate jobs. And at the top is the only layer
I wrote myself.}

\figpara{The three libraries, in one line each.}{\textbf{Transformers} knows how
to load a published model and run a standard training loop over it.
\textbf{PEFT} is what makes training affordable: it freezes the model's 500
million numbers and adds a small set of new ones beside them, which are the only
ones ever modified. \textbf{TRL} contributes nothing conceptual at all; I use it
for the format its files are written in, which is why it survived to the end
while everything else in its toolbox was replaced.}

\figpara{Why the top layer had to be mine.}{Everything this report is about
lives there. Making a mistake at seven in the evening count more than a mistake
at two in the afternoon, and sending one kind of user to one set of numbers and
another kind to another, cannot be expressed by changing a setting. Both require
reaching inside the training step, and that is the single property the whole
stack was chosen for in the first week.}

\figpara{The column on the right.}{Three standard libraries that never touch the
model. They read and reshape the files, split the people into a training group
and a test group, and run the statistical checks that decide, in
Chapter~\ref{ch:scale}, whether a difference between two runs is real.}}
\end{figure}
