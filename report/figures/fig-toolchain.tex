...
\label{fig:toolchain}
\figtext{%
\figpara{Software toolchain architecture stack of Figure~\ref{fig:toolchain}.}{At the bottom of Figure~\ref{fig:toolchain} is the machine, a
graphics card rented for free through a browser, which is the constraint behind
almost every choice in this chapter. Above it sit the model and the trick that
makes it fit in that card's memory, then three off-the-shelf libraries:
\textbf{Transformers} loads a published model and runs a standard training loop,
\textbf{PEFT} makes training affordable by freezing the model's 500 million
numbers and adding a small set beside them, and \textbf{TRL} contributes only the
format its files are written in. The column on the right never touches the model.}

\figpara{Custom mobility evaluation framework layer in Figure~\ref{fig:toolchain}.}{Everything this report is about lives
in the custom top layer of Figure~\ref{fig:toolchain}. Making a mistake at seven in the evening count more than one at two in the
afternoon, and sending one kind of user to one set of numbers and another kind to
another, cannot be expressed by changing a setting. Both require reaching inside
the training step, and that is the single property the whole stack was chosen for
in the first week.}}
