...
distances are equal multiples and 500 to 2{,}000 occupies the same width as
5{,}000 to 20{,}000; the vertical axis is not accuracy but the gap to the
baseline, the only quantity that survives a change of test set, so the dashed
line at zero means ``indistinguishable from repeating the last cell''. At 500
users the model gains $+0.6\pt$ and is barely distinguishable from the rule it
is meant to beat. Between 500 and 2{,}000 the edge climbs $+0.6 \rightarrow
...
down, it stops. Two things constrain how far that reads. The 500 to
2{,}000-user tiers are scored on \stwok{} (100 test users, 48.2\,\% baseline;
the 500-user point is the \smob{} run, 43.1\,\% baseline) and the 5{,}000 to
...
read: the defensible claim is that 2{,}000 users are enough at this training
budget, not that more users can never help.}
