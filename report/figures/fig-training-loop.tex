...
surprised the model was, and that number is used to nudge the model's adjustable
numbers in the direction that would have made it less wrong. Only a small part
ever moves: the 500 million numbers the model was published with are frozen, and
the nudging applies to about 2 in every 100 added beside them.}
...
population and starts learning the days by heart. Its score on people it has
never seen then turns around and falls while its score on its own training data
keeps improving, so the training figures alone will happily report success for
another forty passes. Grading on separate people after each pass, and keeping the
best copy rather than the last, was the largest single improvement of the whole
internship.}}
\end{figure}
