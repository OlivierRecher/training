...
\end{groupplot}
\end{tikzpicture}}
\caption[The hour-of-day ablation]{Does the model actually read the hour?
(a)~Each event enters the sequence as a dedicated hour token followed by its
\cid{cell\_id}, and the target's loss is multiplied inside the configured
transition windows. (b)~Same test users, same cell sequence, only the hour
token corrupted: destroying the \emph{ordering} of the hours costs
$-3.0\pt$, four times the penalty of replacing every hour by a constant, so
the token carries real information rather than merely occupying a position.
(c)~Shifting every hour by $+12$h barely moves the aggregate but swaps the two
halves of the day: accuracy inside the transition windows falls
$42.7 \rightarrow 39.7\,\%$ while accuracy outside them rises
$45.3 \rightarrow 47.1\,\%$. \smob, 80\,\% context, $n = 1{,}747$
predictions.}
