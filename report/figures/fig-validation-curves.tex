...
\end{groupplot}
\end{tikzpicture}}
\caption[Validation accuracy per epoch on four datasets]{The same shape, four
times, on four different datasets: validation top-1 climbs for a handful of
epochs, peaks (star), then decays --- and the plateau sits \emph{below} the
dashed persistence baseline of that dataset. Note that the baseline is a
property of the data, not of the run: it is 69.2\,\% on the unfiltered day and
43.5\,\% once the corpus is restricted to mobile users, because the users
themselves are different. Per-epoch values are recovered by digitising each
run's own validation curve, with the vertical axis calibrated on the two
values printed on that curve, so an individual point carries about
$\pm1\pt$ of reading error; the figures quoted in the text are the reported
ones. Qwen2.5-0.5B + QLoRA throughout.}
