...
\begin{scope}[shift={(3.5,0)}]
  ...
  \node[ttl] at (0,2.95) {(a) One mast carries four 2G cells};
...
\begin{scope}[shift={(10.5,0)}]
  ...
  \node[ttl] at (0,2.95) {(b) The same mast also carries 3G: eight cells};
  \node[scap,text width=5.4cm] at (0,-2.95)
    {the smaller lobes are the 2G cells of panel (a)};
\end{scope}
...
\figtext{%
\figpara{What is drawn, and why it decides how movement is measured.}{Both panels
are the same mast of the study area seen from above, the crossed circle being the
installation itself and each shaded lobe the area covered by one of its antennas
pointing in one direction. Panel (a) has the four 2G cells alone; panel (b) adds
the four 3G ones, which reach differently and point elsewhere. The eight lobes all
start at the same point, so any hop among those eight names happens at one
address. On the day used for Chapter~\ref{ch:groups} that is \textbf{12.4 out of
every 100} back-to-back records. Movement is therefore always measured on the
mast, while what has to be \emph{predicted} stays the full antenna name.}}
