...
number in them, which means keeping three further numbers in GPU memory beside
each one for as long as the run lasts, several times the size of the model. (b)~LoRA \cite{hu2022lora} leaves the big matrix
untouched and adds two thin matrices beside it, whose product has the same
shape; only those are trained. Two narrow matrices standing in for a change to
a wide one is the whole of the idea, and here it amounts to about 2\,\% of the
parameters. (c)~QLoRA \cite{dettmers2023qlora}, the regime
