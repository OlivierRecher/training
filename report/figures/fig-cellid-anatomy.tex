...
middle letters name the mast, and the last digits say which antenna of that mast
served the phone. Only the middle part corresponds to a place on a map, so the
digits change without anybody going anywhere. On the right, the same mast in the
town of Cheb appears under four different names. On the day used for
Chapter~\ref{ch:groups}, \textbf{12.4 out of every 100} back-to-back records are
exactly that: a new name, the same mast, a person who did not move. Counting them
as movement would inflate every mobility figure here by about a quarter, so
